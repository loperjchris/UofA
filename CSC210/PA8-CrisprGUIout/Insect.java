...
public class Insect extends Animal {
    ...
    public void takeStep() {
    	// Checks to see if the animal hasn't died from age.
    ...
    // Sets the next direction for Insects if they're going clockwise.
    ...
    // Sets the next direction for Insects if they're going counter clockwise.
    public void changeDirectionCounter() {
        if (this.getNextDirection().equals("left")) {
            this.setNextDirection("down");
    ...
    public void reproduce(Insect parent, Grid grid) {
        ...
        if (!this.getType().equals("mosquito")) {
            ...
            List<Boolean> states = Arrays.asList(true, false);
            int gender = rand.nextInt(2);
            int clockwise = rand.nextInt(2);
            Animal baby = new Insect(this.getX(), this.getY(), this.getType(),
                    genders.get(gender), states.get(clockwise));
