...
/*
* This method allocates enough entries for the number of pages
* given.
* Returns The table of page entries.
*/
static USLOSS_PTE *
PageTableAllocateIdentity(int pages)
{
    USLOSS_PTE  *table = NULL;
    // Check if the process has been initialized yet
    if (initialized) {
        table = malloc(sizeof(USLOSS_PTE) * pages);
        // Set the entries attributes
...
/*
* This method frees all the entries for every page of a process.
* Returns the result of whether or not the pages could be freed.
*/
static int
PageTableFree(PID pid)
{
    ...
    if (initialized) {
        ...
        if (pid < 0 || pid > P1_MAXPROC) {
            result = P1_INVALID_PID;
        // Check if table has been initialized for process with PID
        } else if (pageTables[pid] == NULL) {
            result = P1_INVALID_PID;
        // Free memory allocated for pages and set to NULL
        } else {
            free(pageTables[pid]);
            pageTables[pid] = NULL;
        }
